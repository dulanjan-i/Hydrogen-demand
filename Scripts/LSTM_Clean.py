...
forecasted_bau.to_csv('forecasted_BAU.csv', index=False)
print("Exported forecasted BAU data to 'forecasted_BAU.csv'")

###############################################################################

# Re-import forecasted BAU data and t45 predictions
forecasted_bau = pd.read_csv('forecasted_BAU.csv')
...
merged_df.to_csv('final_t45_results.csv', index=False)

print("Predictions saved to 'final_t45_results.csv'.")
...
energy_y_true = merged_df['Forecasted_Energy_Consumption']  
energy_y_pred = merged_df['Forecasted_Energy_Consumption']  # Self-comparison for energy model
evaluate_model(energy_y_true, energy_y_pred, "Energy Model")

# Historical CO2 Model Evaluation
historical_co2_y_true = merged_df['CO2_Predicted_from_Historical']  
historical_co2_y_pred = merged_df['CO2_Predicted_from_Historical']  # Self-comparison for historical model
evaluate_model(historical_co2_y_true, historical_co2_y_pred, "Historical CO2 Model")

# Energy-Based CO2 Model Evaluation
energy_based_co2_y_true = merged_df['CO2_Predicted_from_BAU']  
energy_based_co2_y_pred = merged_df['CO2_Predicted_from_BAU']  # Self-comparison for energy-based model
evaluate_model(energy_based_co2_y_true, energy_based_co2_y_pred, "Energy-Based CO2 Model")

# T45 Electricity CO2 Model Evaluation
t45_electricity_y_true = merged_df['CO2_Predicted_from_BAU']  # Compare against BAU predictions
t45_electricity_y_pred = merged_df['Remaining_CO2_Balance_t45_electricity']
evaluate_model(t45_electricity_y_true, t45_electricity_y_pred, "T45 Electricity CO2 Model")

# T45 Hydrogen CO2 Model Evaluation
t45_hydrogen_y_true = merged_df['CO2_Predicted_from_BAU']  # Compare against BAU predictions
t45_hydrogen_y_pred = merged_df['Remaining_CO2_Balance_t45_hydrogen']
evaluate_model(t45_hydrogen_y_true, t45_hydrogen_y_pred, "T45 Hydrogen CO2 Model")

# T45 Red_Eff CO2 Model Evaluation
t45_red_eff_y_true = merged_df['CO2_Predicted_from_BAU']  # Compare against BAU predictions
t45_red_eff_y_pred = merged_df['Remaining_CO2_Balance_t45_red_eff']
evaluate_model(t45_red_eff_y_true, t45_red_eff_y_pred, "T45 Red_Eff CO2 Model")
...
# Create a separate DataFrame for the baseline year
baseline_year = 2018
baseline_df = historical_data[historical_data['Year'] == baseline_year].copy()

# Merge baseline data with predictions for evaluation
baseline_merged = pd.merge(
    baseline_df,
    merged_df[merged_df['Year'] == baseline_year],
    on=['State'],
    suffixes=('_true', '_pred')
)

# Energy Model Evaluation
energy_y_true = baseline_merged['Energy_Consumption_true']
energy_y_pred = baseline_merged['Forecasted_Energy_Consumption_pred']
evaluate_model(energy_y_true, energy_y_pred, "Energy Model (Baseline Year)")

# Historical CO2 Model Evaluation
historical_co2_y_true = baseline_merged['CO2_Balances_true']
historical_co2_y_pred = baseline_merged['CO2_Predicted_from_Historical_pred']
evaluate_model(historical_co2_y_true, historical_co2_y_pred, "Historical CO2 Model (Baseline Year)")

# Energy-Based CO2 Model Evaluation
energy_based_co2_y_true = baseline_merged['CO2_Balances_true']
energy_based_co2_y_pred = baseline_merged['CO2_Predicted_from_BAU_pred']
evaluate_model(energy_based_co2_y_true, energy_based_co2_y_pred, "Energy-Based CO2 Model (Baseline Year)")

# T45 Electricity CO2 Model Evaluation
t45_electricity_y_true = baseline_merged['CO2_Balances_true']
t45_electricity_y_pred = baseline_merged['Remaining_CO2_Balance_t45_electricity']
evaluate_model(t45_electricity_y_true, t45_electricity_y_pred, "T45 Electricity CO2 Model (Baseline Year)")

# T45 Hydrogen CO2 Model Evaluation
t45_hydrogen_y_true = baseline_merged['CO2_Balances_true']
t45_hydrogen_y_pred = baseline_merged['Remaining_CO2_Balance_t45_hydrogen']
evaluate_model(t45_hydrogen_y_true, t45_hydrogen_y_pred, "T45 Hydrogen CO2 Model (Baseline Year)")

# T45 Red_Eff CO2 Model Evaluation
t45_red_eff_y_true = baseline_merged['CO2_Balances_true']
t45_red_eff_y_pred = baseline_merged['Remaining_CO2_Balance_t45_red_eff']
evaluate_model(t45_red_eff_y_true, t45_red_eff_y_pred, "T45 Red_Eff CO2 Model (Baseline Year)")
